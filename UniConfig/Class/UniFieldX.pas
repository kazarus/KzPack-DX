...
begin
  Result:='SELECT SYSCOLUMNS.COLID AS COLINDEX,SYSCOLUMNS.NAME AS COLVNAME,SYSCOLUMNS.XTYPE AS COLVTYPE,SYSTYPES.NAME AS TYPENAME,SYSCOLUMNS.PREC AS DATASIZE,SYSCOLUMNS.SCALE AS DATASCAL,'
         ...
         +'        AND INDID IN (SELECT INDID FROM SYSINDEXES WHERE 1=1 AND NAME  IN (SELECT NAME FROM SYSOBJECTS WHERE  1=1  AND PARENT_OBJ=OBJECT_ID($TABL)))) '//@XTYPE=$PK
...
begin
  Result := 'SELECT A.NAME AS COLVNAME,B.NAME AS TABLNAME,A.PREC AS DATASIZE,A.SCALE AS DATASCAL,C.NAME AS TYPENAME FROM SYSCOLUMNS A'
          + '    LEFT JOIN SYSOBJECTS B ON A.ID=B.ID'
          + '    INNER  JOIN SYSTYPES  C ON A.XTYPE=C.XTYPE AND C.XUSERTYPE<256'
          + '    WHERE B.TYPE=%S'
          + '    ORDER BY B.NAME';
