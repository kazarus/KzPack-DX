...
{$ENDIF}
  ...
  public
    ...
    property AutoFontSize:Boolean read FAutoFontSize write FAutoFontSize default False;
...
begin
  ...
  begin
    ...
    frxDefaultConnection.ConnectionObject._AddRef;
...
function TfrxComponent.Get_Visible(out Value: WordBool): HResult; stdcall;
begin
  Value := Visible;
  Result := S_OK;
end;

function TfrxComponent.Set_Visible(Value: WordBool): HResult; stdcall;
begin
  Visible := Value;
  Result := S_OK;
end;
...
var
  ...
  //YXC_2010_05_31_17_26_48
  SizeA    : Integer;
  WidthA   : Extended;
  HeighA   : Extended;
begin

  //YXC_2010_05_31_17_27_46
  if (AutoFontSize) and (not IsDesigning) then
  begin
    SizeA :=Font.Size;
    WidthA:=Width;
    HeighA:=Height;

    while (WidthA * HeighA) <= (CalcWidth * CalcHeight) do
    begin
      SizeA:=SizeA -1;
      Font.Size:=SizeA;
    end;
...
//      if DataSets[i].DataSet = nil then
    ...
    begin
      ...
      if Ref <> nil then
        SetOrdProp(lItem.Obj, lItem.PropInfo, Integer(Ref));
      lItem.Free;
      FixupList.Delete(0);
    end;
  end
