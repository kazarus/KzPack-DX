...
//  val: Variant;
  ...
  begin
    Result := Report.Calc(aExp);
    if (TVarData(Result).VType = varString) or (TVarData(Result).VType = varOleStr)
    {$IFDEF Delphi12} or (TVarData(Result).VType = varUString){$ENDIF} then
...
begin
  ...
  if Frame.DropShadow then
    begin
      bx1 := FX + Round((Width - Frame.ShadowWidth) * ScaleX);
      by1 := FY + Round((Height - Frame.ShadowWidth) * ScaleY);
    end
  else
    begin
      bx1 := FX + Round(Width * ScaleX);
      by1 := FY + Round(Height * ScaleY);
    end;
