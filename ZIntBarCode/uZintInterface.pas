...
begin
  ...
  Result := ExcludeTrailingPathDelimiter(String(p));
...
zint_dll_file := SystemPath +'\'+LibName;
if not FileExists(zint_dll_file) then
begin
  rs := TResourceStream.Create(HInstance, 'ZINT_DLL', RT_RCDATA);
  try
    rs.SaveToFile(zint_dll_file);
  finally
    rs.Free;
  end;
end;
