...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, VirtualTrees, Grids, ValEdit, qxml, ComCtrls, Vcl.Buttons,
  Vcl.Imaging.jpeg, Vcl.Menus, Clipbrd;
...
type
  TfrmMain = class(TForm)
    ...
    procedure vstXMLNodeClick(Sender: TBaseVirtualTree;
...
implementation
uses qstring,ExtActns,openurl,iduri,find,Class_KzUtils;
{$R *.dfm}
...
procedure TfrmMain.Btnv_1Click(Sender: TObject);
begin
  TKzUtils.Explore;
end;
...
begin
  Clipbrd.Clipboard.AsText:=FNodeName;
end;
...
procedure TfrmMain.vstXMLNodeClick(Sender: TBaseVirtualTree;
  const HitInfo: THitInfo);
begin
  //#
end;
